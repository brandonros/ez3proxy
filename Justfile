...
bootstrap:
    ...
    # Prepare host key for agenix decryption
    tmp=$(mktemp -d)
    install -d -m 755 "$tmp/etc/ssh"
    install -m 600 secrets/host-key "$tmp/etc/ssh/ssh_host_ed25519_key"
    install -m 644 secrets/host-key.pub "$tmp/etc/ssh/ssh_host_ed25519_key.pub"
